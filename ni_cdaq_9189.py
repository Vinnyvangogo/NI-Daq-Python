...
class DAQManager:
    ...
    def __init__(self, chassis_name: str, ip: str, error_queue: "queue.Queue"):
        ...
        # Per-channel RMS mode flags for the 9320 modules (2-6). True (default,
        # matches original behavior) = report true RMS (sqrt-mean-square) of
        # the sample block. False = report the plain averaged (DC) voltage of
        # the block instead -- useful for channels that aren't AC signals.
        # This is independent of ai9320_enabled: a channel can still be
        # sampled while its measurement mode is switched between RMS and raw.
        self.ai9320_rms_enabled = [True] * AI_9320_TOTAL
        ...
        try:
            while self.ai9320_running:
                ...
                if SIMULATION_MODE:
                    ...
                    # meaningful non-zero value in demo mode. For channels
                    # switched to raw/DC mode, simulate a slowly-varying DC
                    # level instead so the two modes look visibly different.
                    t_now = time.perf_counter()
                    raw_meas = []
                    for i in range(AI_9320_TOTAL):
                        if self.ai9320_rms_enabled[i]:
                            v = (abs(5.0 * math.sin(t_now * 2 * math.pi * 60 + i * 0.3))
                                 * (1 + np.random.randn() * 0.01))
                        else:
                            v = (2.0 * math.sin(t_now * 0.05 + i * 0.3)
                    ...
                    try:
                        ...
                        # depending on ai9320_rms_enabled[i], then apply
                ...
                # Apply calibration. RMS-mode channels are clamped to zero
                # since true RMS is always non-negative by definition (sqrt
                # of mean of squares) -- a small negative offset in the cal
                # table can otherwise produce negative values near zero which
                # are physically meaningless for AC voltage/current RMS.
                # Raw/DC-mode channels are NOT clamped, since a plain averaged
...
class DAQApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Per-channel RMS/raw mode for the 9320 modules, loaded from JSON
        # "rms_enabled" before the UI is built (same pattern as "enabled").
        self._json_9320_rms_enabled = [True] * AI_9320_TOTAL
    ...
    def _build_9320_tab(self):
        ...
        ttk.Button(ctrl, text="RMS All", style="A.TButton",
                   command=lambda: self._set_all_ai9320_rms(True)).pack(side="left", padx=(10, 2))
        ttk.Button(ctrl, text="Raw All", style="R.TButton",
                   command=lambda: self._set_all_ai9320_rms(False)).pack(side="left", padx=2)
        ...
        self._ai9320_rms_checks: list[tk.BooleanVar] = []
        self._ai9320_unit_lbls: list[tk.Label] = []
        ...
        for mod in range(5):
            ...
            tk.Label(col, text=f"Module {mod + 2}  [Vrms / Raw per ch]", font=FONT_BOLD,
            ...
            for ch in range(16):
                ...
                unit_lbl = tk.Label(
                    row,
                    text=("RMS" if self._json_9320_rms_enabled[idx] else "Raw"),
                    font=FONT_TINY, width=3,
                    bg=C_PANEL,
                    fg=(C_ACCENT if self._json_9320_rms_enabled[idx] else C_YELLOW))
                unit_lbl.pack(side="left", padx=(0, 2))
                self._ai9320_unit_lbls.append(unit_lbl)

                rms_var = tk.BooleanVar(value=self._json_9320_rms_enabled[idx])
                self._ai9320_rms_checks.append(rms_var)
                ttk.Checkbutton(row, variable=rms_var, text="RMS",
                                style="Panel.TCheckbutton",
                                command=lambda i=idx: self._ai9320_rms_toggle(i)
                                ).pack(side="left")
    ...
    def _sync_ui_to_daq(self):
        ...
        for i, chk in enumerate(self._ai9320_rms_checks):
            self.daq.ai9320_rms_enabled[i] = chk.get()
    ...
    def _ai9320_rms_toggle(self, idx):
        """Switch a single 9320 channel between RMS and raw/DC measurement
        mode, live if connected, and update its unit label."""
        state = self._ai9320_rms_checks[idx].get()
        if self.daq:
            self.daq.ai9320_rms_enabled[idx] = state
        self._ai9320_unit_lbls[idx].config(
            text=("RMS" if state else "Raw"),
            fg=(C_ACCENT if state else C_YELLOW))

    def _set_all_ai9320_rms(self, state: bool):
        for i, chk in enumerate(self._ai9320_rms_checks):
            chk.set(state)
            if self.daq:
                self.daq.ai9320_rms_enabled[i] = state
            self._ai9320_unit_lbls[i].config(
                text=("RMS" if state else "Raw"),
                fg=(C_ACCENT if state else C_YELLOW))
    ...
    def _save_calibration(self):
        ...
        for i in range(AI_9320_TOTAL):
            ...
            channels_9320.append({
                ...
                "rms_enabled": self._ai9320_rms_checks[i].get() if i < len(self._ai9320_rms_checks) else True,
        ...
        try:
            ...
            for rec in data.get("NI_9320_modules_2_to_6", []):
                ...
                if 0 <= idx < len(AI9320_NAMES):
                    ...
                    self._json_9320_rms_enabled[idx] = bool(rec.get("rms_enabled", True))
    ...
    def _open_csv(self):
        ...
        def _9320_suffix(i):
            rms_on = (self._ai9320_rms_checks[i].get()
                      if i < len(self._ai9320_rms_checks) else True)
            return "Vrms" if rms_on else "Vavg"

        ai9320_hdrs = [f"Mod{(i//16)+2}_CH{i%16:02d}_{AI9320_NAMES[i]}_{_9320_suffix(i)}"
                       for i in range(AI_9320_TOTAL)]
